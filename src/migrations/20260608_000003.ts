...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await addColumnIfMissing(db, 'career_applications', 'position_applied_for text')
  await addColumnIfMissing(db, 'career_applications', 'current_salary text')
  await addColumnIfMissing(db, 'career_applications', 'expected_salary text')
  await addColumnIfMissing(db, 'career_applications', 'notice_period text')
}
